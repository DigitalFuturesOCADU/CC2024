...
#define DEVICE_NAME "ButtonDevice_B"  // Change A to B,C,etc for different devices
...
const char* RSI_UUID = "19B10000-E8F2-537E-4F6C-D104768A1006";  // New UUID for RSI
...
// RSI characteristic (signed int8 for dBm values)
BLEIntCharacteristic rsiCharacteristic(RSI_UUID, BLERead | BLENotify);
...
// RSI update interval
const long RSI_UPDATE_INTERVAL = 1000;  // Update RSI every second
unsigned long lastRsiUpdate = 0;
...
void setup() {
 ...
 // while (!Serial) delay(10);  // Wait for serial connection
  ...
  // Add RSI characteristic
  buttonService.addCharacteristic(rsiCharacteristic);
  Serial.println("Added RSI characteristic");
...
void loop() {
  ...
  if (isConnected) {
    ...
    // Update RSI value periodically
    if (currentTime - lastRsiUpdate >= RSI_UPDATE_INTERVAL) {
      lastRsiUpdate = currentTime;
      int rsi = BLE.rssi();  // Get current RSI value
      rsiCharacteristic.writeValue(rsi);
      Serial.print("RSI updated: ");
      Serial.println(rsi);
    }
  }
}
